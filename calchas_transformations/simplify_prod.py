...
from calchas_datamodel import Prod
from calchas_datamodel.dummy import DummyGen
from .transformation import Transformation
from typing import Union


class SimplifyProd(Transformation):
    def __init__(self, prefix: str = "_simplify_prod_", suffix: str = ""):
        self.prefix = prefix
        self.suffix = suffix
    ...
    def apply(self, tree: AbstractExpression, dummy_gen: DummyGen) -> AbstractExpression:
        return SimplifyProdVisitor(tree, dummy_gen).visit()


class SimplifyProdVisitor(AbstractVisitor):
    ...
    def _visit_call(self, tree: Call, *args):
        ...
        if isinstance(tree, Prod) or tree.fun == GenericConstantFunction("Prod"):
            l = []
            u = False
            for e in tree.args:
                if isinstance(e, Prod) or (isinstance(e, Call) and e.fun == GenericConstantFunction("Prod")):
                    u = True
                    for a in e.args:
                        l.append(a)
                elif e != Int(1):
